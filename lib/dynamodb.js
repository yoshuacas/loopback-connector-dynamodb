...
DynamoDB.prototype.define = function (modelDefinition){
	debug ("DEFINE", modelDefinition.model.modelName, ":",modelDefinition.settings, "Properties :", modelDefinition.properties);
	...
	var getKeyHashAndRange = helper.getKeyHashAndRange;
  	...
  	if (modelDefinition.idInjection) {
  		...
  		var keys = getKeyHashAndRange(properties);

  		if (!keys){
  			//Your keys definition is incorrect. Table creation with Dynamo will fail, maybe that is enough message for the developer
  			//there is Debug message in the helper class also
  		} else if (keys.length === 0){
  			//We will create a key for the property 'id' of type Partition, validate first that idInjection should be true
  			//If idInjection is not tru, DynamoDB will fail for having no Keys
  			if (modelDefinition.settings.idInjection===true){
  				createTableParams.KeySchema.push (
  					{
				      AttributeName: 'id',
				      KeyType: 'HASH'
  			...
  			//Create the table with the provided Partition and Sort keys
  		...
  		Object.keys(properties).forEach (function(key){
  			...
  			createTableParams.AttributeDefinitions.push ({
	  			AttributeName: key,
	      		AttributeType: properties[key].type.name
  			});

  		});

  		debug (getKeyHashAndRange(properties));
